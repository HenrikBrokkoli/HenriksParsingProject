...
pub fn make_memberset(chrs: &str) -> HashSet<SetMemberWithEmpty> {
    ...
    for x in chrs.chars() {
        if x == '#' {
            ...
            set.insert(SetMemberWithEmpty::Terminate);
        } else {
            set.insert(SetMemberWithEmpty::Char(x));
        }
    }
    return set;
}
...
pub fn make_memberset_no_empty(chrs: &str) -> HashSet<SetMember> {
    ...
    for x in chrs.chars() {
        if x == '!' {
            set.insert(SetMember::Terminate);
        } else {
            set.insert(SetMember::Char(x));
        }
    }
    return set;
}
...
mod tests {
    ...
    fn test_make_memberset() {
        ...
        assert_eq!(
            HashSet::<SetMemberWithEmpty>::from([
                SetMemberWithEmpty::Char('a'),
                SetMemberWithEmpty::Char('b'),

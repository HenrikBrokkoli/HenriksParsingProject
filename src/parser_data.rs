    ...
    T: VM,
{
    ...
    pub fn get_elements_verbose(&self) -> Vec<ElementVerbose> {
        ...
        for (ix, element_data) in self.element_data.iter().enumerate() {
            elements_verbose.push(ElementVerbose::new(
                element_data.name.clone(),
                self.element_types[ix],
            ))
        }
        elements_verbose
    }
    ...
    pub fn get_or_add_element_key(&mut self, identifier: &ElementVerbose) -> ElementIndex {
        match self.element_verbose_map.get(identifier) {
            None => {
...
impl<T> fmt::Debug for ParseRules<T>
where
    T: VM,
{

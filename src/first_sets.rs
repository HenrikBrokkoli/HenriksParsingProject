...
) -> Result<HashSet<SetMemberWithEmpty>, GrammarError>
where
    T: VM,
{
...
pub fn get_first_sets<T>(parser_data: &ParserData<T>) -> Result<NamedSets, GrammarError>
where
    T: VM,
{
...
fn get_set_first_set_of_element<T>(
    el_ix: ElementIndex,
    first_sets: &mut NamedSets,
    parser_data: &ParserData<T>,
) -> Result<HashSet<SetMemberWithEmpty>, GrammarError>
where
    T: VM,
...
fn get_first_set_of_element<T>(
    el_ix: ElementIndex,
    first_sets: &NamedSets,
    parser_data: &ParserData<T>,
) -> Result<Option<HashSet<SetMemberWithEmpty>>, GrammarError>
where
    T: VM,
{
    let et = parser_data.element_types[el_ix];
    let res = match et {
        ElementType::NonTerminal => first_sets.get(&el_ix).map(|entry| (*entry).clone()),
...
fn get_first_set_of_production<T>(
    ...
    T: VM,
{
    match production {
        ...
        Production::NotEmpty(el_ixs) => {
            let mut first_menge =
                get_set_first_set_of_element(*el_ixs.first().unwrap(), first_sets, parser_data)?;
...
";
        ...
        assert_eq!(
            ...
            first_dict
                .get(
                    &rule_parser
                        .parser_data
                        .get_element_nt_index("identifier2")
                        .unwrap()
                )
                .unwrap()
                .clone()
        ...
        assert_eq!(
            ...
            first_dict
                .get(
                    &rule_parser
                        .parser_data
                        .get_element_nt_index("identifier3")
                        .unwrap()
                )
                .unwrap()
                .clone()
    ...
    fn test_first_mengen2() {
        let to_parse = "start      ->  identifier2 identifier3
                            | \"a_terminal\";\
            identifier2 ->  \"b_terminal\"\
                            | #;
            identifier3 -> \"c_terminal\";
...
";
    ...
    fn test_first_set_of_partial() {
        ...
        parser_data.get_or_add_element_key(&ElementVerbose::new(
            String::from("0"),
    ...
    fn test_first_mengen_hard() {
        let to_parse = "start      -> identifier2 list identifier3;\
            identifier2 -> \"b_terminal\"\
                            | #;
            identifier3 -> \"c_terminal\"| #;
...
";
        ...
        assert_eq!(
            make_memberset("lhbgc#"),
            first_dict
                .get(&parser_data.get_element_nt_index("start").unwrap())
                .unwrap()
                .clone()
        );
        assert_eq!(
            make_memberset("c#"),
            first_dict
                .get(&parser_data.get_element_nt_index("identifier3").unwrap())
                .unwrap()
                .clone()
        );
        assert_eq!(
            make_memberset("ghl#"),
            first_dict
                .get(&parser_data.get_element_nt_index("list").unwrap())
                .unwrap()
                .clone()
        );
        assert_eq!(
            make_memberset("#gh"),
            first_dict
                .get(&parser_data.get_element_nt_index("listt").unwrap())
                .unwrap()
                .clone()
        );
        assert_eq!(
            make_memberset("gh"),
            first_dict
                .get(&parser_data.get_element_nt_index("listelement").unwrap())
                .unwrap()
                .clone()
        );
        assert_eq!(
            make_memberset("l#"),
            first_dict
                .get(&parser_data.get_element_nt_index("liststart").unwrap())
                .unwrap()
                .clone()
        );
        assert_eq!(
            make_memberset("b#"),
            first_dict
                .get(&parser_data.get_element_nt_index("identifier2").unwrap())
                .unwrap()
                .clone()
        );
    }

    ...
    for (name, first) in first_sets.iter() {
        ...
        if first.contains(&SetMemberWithEmpty::Empty) {
            let follow = follow_sets
                .get(name)
                .ok_or(GrammarError::MissingFollowSet { index: *name })?;
...
";
        ...
        assert_eq!(
            make_memberset_no_empty("ab!"),
            steuer_dict
                .get(&parser_data.get_element_nt_index("identifier1").unwrap())
                .unwrap()
                .clone()
        );
        assert_eq!(
            make_memberset_no_empty("b!"),
            steuer_dict
...
";
        ...
        assert_eq!(
            make_memberset_no_empty("a"),
            steuer_dict
                .get(&parser_data.get_element_nt_index("element").unwrap())
                .unwrap()
                .clone()
        );
    }
    ...
    fn test_steuer_sets_optional_list() {
        let rules = "start      -> optional_list \"ende\";\
            optional_list-> list|#;
            list -> element list_s ;\
            list_s -> element list_s| #;\
            element -> \"a\";\
";
        ...
        assert_eq!(
            make_memberset_no_empty("ae"),
            steuer_dict
                .get(&parser_data.get_element_nt_index("optional_list").unwrap())
                .unwrap()
                .clone()
        );
    }
...
whitespace -> \" \";";
        ...
        let steuer_dict = get_steuer_sets(&first_dict, &follow_dict).unwrap();
        assert_eq!(
            make_memberset_no_empty("p+-0123456789"),
            steuer_dict
                .get(&parser_data.get_element_nt_index("start").unwrap())
                .unwrap()
                .clone()
        ...
        assert_eq!(
            make_memberset_no_empty("p+-0123456789! "),
            steuer_dict
                .get(&parser_data.get_element_nt_index("terms_s").unwrap())
                .unwrap()
                .clone()
        );
        assert_eq!(
            make_memberset_no_empty("p+-0123456789"),
            steuer_dict
                .get(&parser_data.get_element_nt_index("term").unwrap())
                .unwrap()
                .clone()
        );

        assert_eq!(
            make_memberset_no_empty("0123456789"),
            steuer_dict
                .get(&parser_data.get_element_nt_index("number").unwrap())
                .unwrap()
                .clone()
        );
        assert_eq!(
            make_memberset_no_empty("0123456789p+- "),
            steuer_dict
                .get(&parser_data.get_element_nt_index("number_s").unwrap())
                .unwrap()
                .clone()
        );
        assert_eq!(
            make_memberset_no_empty("0123456789"),
            steuer_dict
                .get(&parser_data.get_element_nt_index("digit").unwrap())
                .unwrap()
                .clone()
        );
        assert_eq!(
            ...
            steuer_dict
                .get(&parser_data.get_element_nt_index("whitespaces").unwrap())
                .unwrap()
                .clone()
        );
    }

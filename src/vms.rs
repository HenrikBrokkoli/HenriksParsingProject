...
pub type Instruction<T> = dyn Fn(&mut Graph<String>,&mut T)->Result<(),String>;
...
pub trait VM  {
    ...
    ///Create a state to hold thew state of the current VM. The Vm does not take care of the state.
...
impl NullVm{
    ...
    pub fn null_instruction(graph:&mut Graph<String>)-> Result<(),String>{
        Ok(())
    }
}


///This VM does nothing. It's just for testing
impl VM for NullVm {
    ...
    fn make_instruction<'a,T>(&'a self,prod_name:&str, to_parse: &mut ParseProcess<T>) -> Result<Box<Instruction<usize>>, String> where T:TPeekable<Item=char>{
        ...
        Ok(Box::new(|graph,state|NullVm::null_instruction(graph)))
    }

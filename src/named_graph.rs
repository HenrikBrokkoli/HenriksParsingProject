...
use crate::named_graph::GraphError::{
    IndexOutOfBounds, NodeAlreadyExistsError, NodeDoesNotExistsError,
};
...
impl<Ta> GraphNamedNodes<Ta> {
    ...
    pub fn add_node(
        &mut self,
        node_name: ElementIndex,
        data: Ta,
    ) -> Result<NodeIndex, GrammarError> {
    ...
    fn get_index_of_node(&self, node_name: ElementIndex) -> Result<NodeIndex, GrammarError> {
        Ok(*self
            .names
            .get(&node_name)
            .ok_or(GrammarError::GraphNodeDoesNotExistsError { node_name })?
            as NodeIndex)
    }
    ...
    pub fn get_node_mut_by_index(
        &mut self,
        index: NodeIndex,
    ) -> Result<&mut NodeData<Ta>, GrammarError> {
        if index > self.graph.nodes.len() - 1 {
            return Err(GrammarError::GraphIndexOutOfBounds { index });
        }
    ...
    pub fn successor_indices(
        &self,
        node_name: ElementIndex,
    ) -> Result<Successors<'_, Ta>, GrammarError> {
        let index = self.get_index_of_node(node_name)?;
    ...
    pub fn successors(
        &self,
        node_name: ElementIndex,
    ) -> Result<SuccessorsData<'_, Ta>, GrammarError> {
        let index = self.get_index_of_node(node_name)?;
